...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader,
        ...
        test_y_table_header = ["y_label ", "y_pred"]

        self.val_table = PrettyTable(valid_metric_header)
        self.test_table = PrettyTable(test_metric_header)
        self.test_y_table = PrettyTable(test_y_table_header)
    ...
    def save_result(self):
        ...
        test_y_file = os.path.join(self.output_dir, "test_y.txt")
        train_prettytable_file = os.path.join(self.output_dir, "train_markdowntable.txt")
        with open(val_prettytable_file, 'w') as fp:
            fp.write(self.val_table.get_string())
        with open(test_prettytable_file, 'w') as fp:
            fp.write(self.test_table.get_string())
        with open(test_y_file, 'w') as fp:
            fp.write(self.test_y_table.get_string())
    ...
    def test(self, dataloader="test"):
        ...
        if (dataloader == "test"):
            for i in range(0, len(y_label)):
                my_test_lst_ylabel = list(map(str, [y_label[i], y_pred[i]]))
                self.test_y_table.add_row(my_test_lst_ylabel)
