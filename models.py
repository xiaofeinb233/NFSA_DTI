...
class NFSADTI(nn.Module):
    def __init__(self, **config):
        super(NFSADTI, self).__init__()
        ...
        self.drug_extractor = MolecularNFGNN()
...
class MolecularNFGNN(nn.Module):
    def __init__(self, in_feats=75, dim_embedding=128, padding=True, hidden_feats=[128, 128, 128], activation=None):
        super(MolecularNFGNN, self).__init__()
        self.init_transform = nn.Linear(in_feats, dim_embedding, bias=False)
        if padding:
            with torch.no_grad():
                self.init_transform.weight[-1].fill_(0)
        self.gnn = NFGNN(in_feats=dim_embedding, hidden_feats=hidden_feats, activation=activation)
...
class ProteinCNN(nn.Module):
    def __init__(self, embedding_dim, num_filters, kernel_size, padding=True, num_heads=2, dropout=0.1):
    ...
    def forward(self, v):
        ...
        v = v.permute(1, 2, 0)
        v = v.reshape(v.size(0), v.size(2), -1)
        return v
